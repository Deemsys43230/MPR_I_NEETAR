...
public class AppLocker: UIViewController {
    ...
    // var delegate: AppLockerDelegate?
...
//  @IBOutlet weak var photoImageView: UIImageView!
  ...
  fileprivate var mode: ALMode? {
    didSet {
      ...
      case .validate:
        submessageLabel.text = "" // Your submessage for validate mode
  ...
  //  pin == savedPin ? removePin() : incorrectPinAnimation()
    if pin == savedPin {
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "cancelLocker"), object: nil, userInfo: nil)
        removePin()
    } else{
        incorrectPinAnimation()
  ...
  private func validateModeAction() {
    if pin == savedPin {
        ...
        dismiss(animated: true) {
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "successLocker"), object: nil, userInfo: nil)
        }
    } else{
        incorrectPinAnimation()
